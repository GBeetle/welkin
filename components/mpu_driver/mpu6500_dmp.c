...
#define get_ms(x) ((*x) = esp_timer_get_time())
...
int mpu_reset_fifo(struct mpu *mpu)
{
    ...
    if (mpu->writeBytes(mpu, FIFO_EN, 1, &data))
        return -1;
    if (mpu->writeBytes(mpu, USER_CTRL, 1, &data))
        return -1;
 ...
 *  @return     0 if successful.
 */
int mpu_load_firmware(struct mpu *mpu, unsigned short length, const unsigned char *firmware,
...
{
    return mpu_load_firmware(mpu, DMP_CODE_SIZE, dmp_memory, sStartAddress,
        DMP_SAMPLE_RATE);
}
...
int dmp_set_orientation(struct mpu *mpu, unsigned short orient)
{
    ...
    if (mpu_write_mem(mpu, FCFG_1, 3, gyro_regs))
        return -1;
    if (mpu_write_mem(mpu, FCFG_2, 3, accel_regs))
        return -1;
    ...
    if (mpu_write_mem(mpu, FCFG_3, 3, gyro_regs))
        return -1;
    if (mpu_write_mem(mpu, FCFG_7, 3, accel_regs))
        return -1;
    dmp.orient = orient;
    return 0;
}
...
int dmp_set_gyro_bias(struct mpu *mpu, long *bias)
{
    ...
    if (mpu_write_mem(mpu, D_EXT_GYRO_BIAS_X, 4, regs))
        return -1;
    ...
    if (mpu_write_mem(mpu, D_EXT_GYRO_BIAS_Y, 4, regs))
        return -1;
    ...
    return mpu_write_mem(mpu, D_EXT_GYRO_BIAS_Z, 4, regs);
}
...
int dmp_set_accel_bias(struct mpu *mpu, long *bias)
{
    ...
    accel_sens = accelSensitivity(accel_fs);
    ...
    return mpu_write_mem(mpu, D_ACCEL_BIAS, 12, regs);
}
...
int dmp_set_fifo_rate(struct mpu *mpu, unsigned short rate)
{
    ...
    if (mpu_write_mem(mpu, D_0_22, 2, tmp))
        return -1;
    if (mpu_write_mem(mpu, CFG_6, 12, (unsigned char*)regs_end))
        return -1;
...
int dmp_set_tap_thresh(struct mpu *mpu, unsigned char axis, unsigned short thresh)
{
    ...
    accel_fsr = accelFSRvalue(accel_fs);
    switch (accel_fsr) {
    case 2:
    ...
    if (axis & TAP_X) {
        if (mpu_write_mem(mpu, DMP_TAP_THX, 2, tmp))
            return -1;
        if (mpu_write_mem(mpu, D_1_36, 2, tmp+2))
            return -1;
    }
    if (axis & TAP_Y) {
        if (mpu_write_mem(mpu, DMP_TAP_THY, 2, tmp))
            return -1;
        if (mpu_write_mem(mpu, D_1_40, 2, tmp+2))
            return -1;
    }
    if (axis & TAP_Z) {
        if (mpu_write_mem(mpu, DMP_TAP_THZ, 2, tmp))
            return -1;
        if (mpu_write_mem(mpu, D_1_44, 2, tmp+2))
            return -1;
    }
    return 0;
}
...
int dmp_set_tap_axes(struct mpu *mpu, unsigned char axis)
{
    ...
    if (axis & TAP_Z)
        tmp |= 0x03;
    return mpu_write_mem(mpu, D_1_72, 1, &tmp);
}
...
int dmp_set_tap_count(struct mpu *mpu, unsigned char min_taps)
{
    ...
    tmp = min_taps - 1;
    return mpu_write_mem(mpu, D_1_79, 1, &tmp);
}
...
int dmp_set_tap_time(struct mpu *mpu, unsigned short time)
{
    ...
    return mpu_write_mem(mpu, DMP_TAPW_MIN, 2, tmp);
}
...
int dmp_set_tap_time_multi(struct mpu *mpu, unsigned short time)
{
    ...
    tmp[0] = (unsigned char)(dmp_time >> 8);
    tmp[1] = (unsigned char)(dmp_time & 0xFF);
    return mpu_write_mem(mpu, D_1_218, 2, tmp);
}
...
int dmp_set_shake_reject_thresh(struct mpu *mpu, long sf, unsigned short thresh)
{
    ...
    return mpu_write_mem(mpu, D_1_92, 4, tmp);
}
...
int dmp_set_shake_reject_time(struct mpu *mpu, unsigned short time)
{
    ...
    tmp[0] = time >> 8;
    tmp[1] = time & 0xFF;
    return mpu_write_mem(mpu, D_1_90,2,tmp);
}
...
int dmp_set_shake_reject_timeout(struct mpu *mpu, unsigned short time)
{
    ...
    tmp[0] = time >> 8;
    tmp[1] = time & 0xFF;
    return mpu_write_mem(mpu, D_1_88,2,tmp);
}
...
int dmp_get_pedometer_step_count(struct mpu *mpu, unsigned long *count)
{
    ...
    if (mpu_read_mem(mpu, D_PEDSTD_STEPCTR, 4, tmp))
        return -1;
...
int dmp_set_pedometer_step_count(struct mpu *mpu, unsigned long count)
{
    ...
    return mpu_write_mem(mpu, D_PEDSTD_STEPCTR, 4, tmp);
}
...
int dmp_get_pedometer_walk_time(struct mpu *mpu, unsigned long *time)
{
    ...
    if (mpu_read_mem(mpu, D_PEDSTD_TIMECTR, 4, tmp))
        return -1;
...
int dmp_set_pedometer_walk_time(struct mpu *mpu, unsigned long time)
{
    ...
    return mpu_write_mem(mpu, D_PEDSTD_TIMECTR, 4, tmp);
}
...
int dmp_enable_feature(struct mpu *mpu, unsigned short mask)
{
    ...
    mpu_write_mem(mpu, D_0_104, 4, tmp);
    ...
    tmp[8] = 0xA3;
    tmp[9] = 0xA3;
    mpu_write_mem(mpu, CFG_15,10,tmp);
    ...
    if (mask & (DMP_FEATURE_TAP | DMP_FEATURE_ANDROID_ORIENT))
        ...
        tmp[0] = 0xD8;
    mpu_write_mem(mpu, CFG_27,1,tmp);

    if (mask & DMP_FEATURE_GYRO_CAL)
        dmp_enable_gyro_cal(mpu, 1);
    else
        dmp_enable_gyro_cal(mpu, 0);
    ...
    if (mask & DMP_FEATURE_SEND_ANY_GYRO) {
        ...
        mpu_write_mem(mpu, CFG_GYRO_RAW_DATA, 4, tmp);
    }

    if (mask & DMP_FEATURE_TAP) {
        /* Enable tap. */
        tmp[0] = 0xF8;
        mpu_write_mem(mpu, CFG_20, 1, tmp);
        dmp_set_tap_thresh(mpu, TAP_XYZ, 250);
        dmp_set_tap_axes(mpu, TAP_XYZ);
        dmp_set_tap_count(mpu, 1);
        dmp_set_tap_time(mpu, 100);
        dmp_set_tap_time_multi(mpu, 500);

        dmp_set_shake_reject_thresh(mpu, GYRO_SF, 200);
        dmp_set_shake_reject_time(mpu, 40);
        dmp_set_shake_reject_timeout(mpu, 10);
    } else {
        tmp[0] = 0xD8;
        mpu_write_mem(mpu, CFG_20, 1, tmp);
    }
    ...
    if (mask & DMP_FEATURE_ANDROID_ORIENT) {
        ...
        tmp[0] = 0xD8;
    mpu_write_mem(mpu, CFG_ANDROID_ORIENT_INT, 1, tmp);

    if (mask & DMP_FEATURE_LP_QUAT)
        dmp_enable_lp_quat(mpu, 1);
    else
        dmp_enable_lp_quat(mpu, 0);

    if (mask & DMP_FEATURE_6X_LP_QUAT)
        dmp_enable_6x_lp_quat(mpu, 1);
    else
        dmp_enable_6x_lp_quat(mpu, 0);
...
int dmp_enable_gyro_cal(struct mpu *mpu, unsigned char enable)
{
    if (enable) {
        ...
        return mpu_write_mem(mpu, CFG_MOTION_BIAS, 9, regs);
    } else {
...
int dmp_enable_lp_quat(struct mpu *mpu, unsigned char enable)
{
    ...
    mpu_write_mem(mpu, CFG_LP_QUAT, 4, regs);

    return mpu_reset_fifo(mpu);
}
...
int dmp_enable_6x_lp_quat(struct mpu *mpu, unsigned char enable)
{
    ...
    mpu_write_mem(mpu, CFG_8, 4, regs);
...
int dmp_set_interrupt_mode(struct mpu *mpu, unsigned char mode)
{
    ...
    case DMP_INT_CONTINUOUS:
        return mpu_write_mem(mpu, CFG_FIFO_ON_EVENT, 11,
